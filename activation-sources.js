(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ActivationSources=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  const REQUIRED_FIRS=['EHAA','EBBU','EDGG','EDWW','EDMM','LFFF','LFEE','LFBB','LFMM','LFRR'];
  const validDate=value=>{const date=new Date(value);return Number.isFinite(date.getTime())?date:null};
  function notamEvidence(payload,refreshStatus,now=new Date(),staleMinutes=60,briefingWindow=null,requiredFirs=REQUIRED_FIRS){
    ...
    const required=[...new Set((Array.isArray(requiredFirs)&&requiredFirs.length?requiredFirs:REQUIRED_FIRS).filter(fir=>REQUIRED_FIRS.includes(fir)))];
    const firStates=Object.fromEntries(REQUIRED_FIRS.map(fir=>[fir,payload?.fir_coverage?.[fir]||(Number.isInteger(payload?.fir_counts?.[fir])?'COMPLETE':null)]));
    const firCoverage=Object.fromEntries(REQUIRED_FIRS.map(fir=>[fir,Number.isInteger(payload?.fir_counts?.[fir])&&payload.fir_counts[fir]>=0&&firStates[fir]==='COMPLETE']));
    ...
    const configuredFirCoverageComplete=required.every(fir=>firCoverage[fir]);
    const requiredFirCoverageUsable=required.every(fir=>Number.isInteger(payload?.fir_counts?.[fir])&&payload.fir_counts[fir]>=0&&['COMPLETE','PARTIAL_MALFORMED_RECORDS'].includes(firStates[fir]));
    ...
    const checks={faaPayloadValid:payloadValid,generatedTimestampValid,refreshSucceeded,refreshTimestampMatchesDataset,refreshCountMatchesDataset,freshnessPassed,firCoverage,firStates,requiredFirs:required,configuredFirCoverageComplete,requiredFirCoverageUsable,briefingWindowCovered,querySourceErrorsAbsent};
    ...
    for(const fir of required)if(!firCoverage[fir])failedReasons.push(`${fir} FAA coverage is ${firStates[fir]||'missing'}`);
    const complete=failedReasons.length===0;
    const sourceUsable=payloadValid&&generatedTimestampValid&&refreshSucceeded&&refreshTimestampMatchesDataset&&refreshCountMatchesDataset&&freshnessPassed&&briefingWindowCovered&&querySourceErrorsAbsent;
    const coverage_status=sourceUsable&&configuredFirCoverageComplete?'COMPLETE':sourceUsable&&requiredFirCoverageUsable?'PARTIAL':'UNKNOWN';
    return{complete,coverage_status,partial_reason:coverage_status==='PARTIAL'?'FAA coverage partial — malformed source record(s) quarantined':null,checks,failedReasons,generated_at_utc:generated?.toISOString()||null,age_minutes:Number.isFinite(age)?age:null,coverage_complete:configuredFirCoverageComplete,refresh_succeeded:refreshSucceeded&&refreshTimestampMatchesDataset&&refreshCountMatchesDataset,briefing_window_covered:briefingWindowCovered,reason:complete?'complete successful FAA NMS classification load for required FIRs and selected briefing window':failedReasons[0]};
  }
