(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.AirspaceSchedule=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function interval(entry,date){
    ...
    const iso=date.toISOString().slice(0,10),variant=entry.periods_by_date?.[iso]||(summerAt(date)?entry.summer:entry.winter);
  ...
  function evaluate(rule,window,evaluatedAt=new Date()){
    ...
    if(entries.some(e=>e.kind==='UDP'&&(!e.periods_by_date||!Array.isArray(e.holiday_dates))||e.excludes_holidays&&!Array.isArray(e.holiday_dates)))return{status:STATUS.UNKNOWN,reason:'UDP/holiday activation calendar is not available for this briefing window.',intervals:[],source_type:'PUBLISHED_SCHEDULE',source_reference:rule.official_source_url,rule_identifier:rule.identifier,evaluated_at:evaluatedAt.toISOString()};
    ...
    const applicable=[];let unresolvedDate=false;
    for(const entry of entries)for(const date of dates){const iso=date.toISOString().slice(0,10),day=date.getUTCDay();if(entry.days&&!entry.days.includes(day)||entry.exclude_dates?.includes(iso)||entry.excludes_holidays&&entry.holiday_dates.includes(iso))continue;if(entry.kind==='UDP'&&!entry.periods_by_date?.[iso]){if(date<window.to&&new Date(date.getTime()+86400000)>window.from)unresolvedDate=true;continue}const raw=interval(entry,date);if(!raw)continue;const from=new Date(Math.max(raw.from,window.from)),to=new Date(Math.min(raw.to,window.to));if(to>from)applicable.push({from,to})}
    const intervals=merge(applicable),covered=intervals.reduce((sum,x)=>sum+(x.to-x.from),0),duration=window.to-window.from;
    const base={intervals:intervals.map(x=>({from:x.from.toISOString(),to:x.to.toISOString()})),source_type:rule.source_type||'PUBLISHED_SCHEDULE',source_identifier:rule.sup_number||rule.identifier,source_reference:rule.source_url||rule.official_source_url,rule_identifier:rule.identifier,evaluated_at:evaluatedAt.toISOString()};
    if(unresolvedDate)return{...base,status:STATUS.UNKNOWN,reason:'UDP/holiday activation calendar is not available for this briefing date.'};
