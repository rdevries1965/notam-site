(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.BriefingPrintState=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  const STORE='soaring-notam-briefing-v1',SCHEMA_VERSION=4;
  function valid(state){const c=state&&state.criteria,coverage=c?.coverage;return Boolean(state&&state.version===SCHEMA_VERSION&&c&&Array.isArray(c.points)&&c.points.length>=2&&c.date&&c.start_local&&c.end_local&&c.from_utc&&c.to_utc&&c.timezone&&Number.isFinite(Number(c.task_distance_km))&&Number.isFinite(Number(c.corridor_km))&&Number.isFinite(Number(c.max_altitude_ft))&&coverage&&['COMPLETE','PARTIAL','UNKNOWN'].includes(coverage.overall)&&Array.isArray(coverage.supportedFirs)&&Array.isArray(coverage.faaFirs)&&state.sources&&Array.isArray(state.group_order)&&state.group_order.every(key=>Array.isArray(state.groups?.[key])))}
